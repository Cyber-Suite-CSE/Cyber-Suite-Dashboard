...
export function DatabaseScanner() {
  ...
  const [argsPayload, setArgsPayload] = useState<Record<string, any> | null>(null)
  ...
  const canNavigateTo = (screenId: string) => {
    switch (screenId) {
      ...
      case "results":
        return connectionStatus === "connected" && !!runId && selectedTests.length > 0 && !!argsPayload
      default:
        return false
    }
  }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={handleNavigation} className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="connection" className="gap-2" disabled={!canNavigateTo("connection")}>
            <Database size={16} />
            <span className="hidden sm:inline">Database Connection</span>
            <span className="sm:hidden">Connection</span>
          </TabsTrigger>
          <TabsTrigger value="configuration" className="gap-2" disabled={!canNavigateTo("configuration")}>
            <Settings size={16} />
            <span className="hidden sm:inline">Scan Configuration</span>
            <span className="sm:hidden">Configuration</span>
          </TabsTrigger>
          <TabsTrigger value="results" className="gap-2" disabled={!canNavigateTo("results")}>
            <BarChart3 size={16} />
