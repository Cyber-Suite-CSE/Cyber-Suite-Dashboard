...
import { HelpCircle, Shield, Lock, Settings, Wifi, SearchIcon } from "lucide-react"
...
export function ScanConfiguration({
  ...
  useEffect(() => {
    async function fetchTests() {
      try {
        const res = await fetch("http://localhost:8080/get-tests")
        if (!res.ok) throw new Error("Failed to fetch tests")
        const data: ScanTest[] = await res.json()
        setScanTests(data)

        if (selectedTests.length === 0) {
          const recommendedIds = data.filter((t) => t.category_tags.includes("recommended")).map((t) => t.id)
          ...
          recommendedIds.forEach((testId) => {
            ...
            if (test?.args) {
              initialArgs[testId] = initializeArgs(test.args ?? [], extraArgs[testId] || {})
            }
          })
    ...
    argsDef.forEach((arg) => {
      ...
      if (arg.type === "object" && arg.properties) {
        values[arg.name] = initializeArgs(
          Object.entries(arg.properties).map(([key, val]: any) => ({ ...val, name: key })),
          extras[arg.name] || {},
        )
      } else if (arg.type === "array") {
        values[arg.name] = extras[arg.name] ?? (arg.items?.default ? [arg.items.default] : [])
      } else {
  ...
  const allCategories = useMemo(() => {
    ...
    scanTests.forEach((t) => t.category_tags.forEach((cat) => categories.add(cat)))
    return ["all", ...Array.from(categories).sort()]
  }, [scanTests])

  const filteredTests = useMemo(() => {
    return scanTests.filter((t) => {
      if (selectedCategory !== "all" && !t.category_tags.includes(selectedCategory)) return false
      const lowerSearch = searchTerm.toLowerCase()
      return t.name.toLowerCase().includes(lowerSearch) || t.details.toLowerCase().includes(lowerSearch)
    })
      ...
      return (
        <div key={key} className="mb-3">
          <label className="block text-sm font-medium mb-1">
            {arg.label || arg.name} {arg.required && <span className="text-red-500">*</span>}
          </label>
          <input
          ...
          />
          {isMissing && <p className="text-destructive text-xs mt-1">This field is required</p>}
        </div>
      )
    }
    ...
    if (arg.type === "array") {
      ...
      return (
        <div key={key} className="mb-3">
          ...
          <Button size="sm" onClick={() => onChange([...arrVal, ""])} disabled={arrVal.length >= 10}>
            Add Item
          </Button>
          {isMissing && <p className="text-destructive text-xs mt-1">At least one item is required</p>}
        </div>
      )
    }

    if (arg.type === "object" && arg.properties) {
      const propsArr = Object.entries(arg.properties).map(([key, val]: any) => ({ ...val, name: key }))
      return (
        <fieldset key={key} className="mb-4 border border-dashed border-border p-3 rounded">
      ...
      if (testDef?.args) {
        for (const arg of testDef.args) {
          ...
          if (arg.required) {
            ...
            if (arg.type === "array" && (!Array.isArray(val) || val.length === 0)) return false
            if (arg.type === "object" && arg.properties) {
              const propsArr = Object.entries(arg.properties).map(([key, v]: any) => ({ ...v, name: key }))
              for (const subArg of propsArr) {
                const subVal = val?.[subArg.name]
                if (subArg.required && (!subVal || (typeof subVal === "string" && subVal.trim() === ""))) return false
              }
            }
          }
        }
      ...
      <Card>
        <CardHeader className="space-y-2">
          <div className="flex items-center gap-2 flex-wrap">
            <CardTitle className="text-lg font-semibold">Security Scan Configuration</CardTitle>
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger>
                ...
                <TooltipContent>
                  <p className="text-sm">Select security tests to run on your database.</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
          <CardDescription>Choose which security tests to run. Use search and category filters below.</CardDescription>
        ...
        <CardContent className="grid gap-6">
          {pagedTests.length === 0 && <p className="text-center text-muted-foreground">No tests found.</p>}
          {pagedTests.map((test) => {
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <Checkbox checked={isSelected} onChange={() => handleTestToggle(test.id)} className="mt-1" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <Icon className="h-5 w-5 text-muted-foreground shrink-0" />
                      <h3 className="font-medium text-card-foreground whitespace-nowrap">{test.name}</h3>
                      <Badge className={severityColors[test.severity]} variant="secondary">
                        {test.severity}
                      </Badge>
                    </div>
                ...
                {isSelected && test.args?.length > 0 && (
                  <div className="mt-4 border-t border-border pt-4" onClick={(e) => e.stopPropagation()}>
                    {test.args.map((arg) =>
                      renderArgInput(test.id, arg, testsArgsValues[test.id]?.[arg.name], (val) =>
                        setTestsArgsValues((prev) => ({
                          ...prev,
                          [test.id]: { ...(prev[test.id] || {}), [arg.name]: val },
                        })),
                      ),
                    )}
                  </div>
          ...
          <div className="flex justify-between items-center mt-6 pt-6 border-t border-border flex-wrap gap-4">
            ...
            <Button
              ...
              disabled={selectedTests.length === 0 || connectionStatus !== "connected" || !areArgsValid()}
              aria-disabled={selectedTests.length === 0 || connectionStatus !== "connected" || !areArgsValid()}
            >
              Run Scan
            </Button>
          </div>
