...
export function DatabaseConnection({ onConnection }: DatabaseConnectionProps) {
  ...
  const handleConnect = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/db-connection-status?${params.toString()}`, {
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <Card>
          ...
          <div className="flex items-center gap-2">
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>Select your database type and provide connection details</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
          <CardDescription>Configure your database connection to begin security scanning</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {selectedDb && (
            <div className="pt-4">
              <Button onClick={handleConnect} className="w-full md:w-auto" disabled={connectionStatus === "testing"}>
